...
def place_ships(board_size):
 ...
 Takes in the parameter board size (2D list).
 ...
 Returns a tuple (user_ships, computer_ships) in a dictionary.'''
    ...
    while len(user_ships)<3: #Loops for 3 user ships
        ...
        if position not in user_ships.values():
            ship_id = "user_ship_"+str(len(user_ships)+1)
   ...
   if hit in affected_areas.keys():
        ...
        for (a_row,a_col) in affected:
            if ((a_row,a_col) in computer_ships.values()):
                ...
                if board[a_row][a_col] != "X":
                    board[a_row][a_col] = "*" #shown when mine did not hit ship

        if no_ships_hit == True and something_hit == False:
            print("\nNo ships were hit!") #Only displays if no ships were hit from mine
            no_ships_hit = False
 ...
 '''
    ...
    while not valid_move:
        try:
            ...
            #Asks for the user input for the coordinates of the missiles
            rows = row-1
            cols = col-1
            ...
            if ((0<=(rows)<board_size) and (0<=(cols)<board_size)) and board[rows][cols] == " ":
                ...
                #Lets the user know that the input was invalid (was out of range or was already taken)
...
def make_computer_move(board, board_size, user_ships, affected_areas, computer_ships):
    ...
    while True:
        ...
        #Makes sure that the spot is valid
        if (board[rows][cols] == ' ' or board[rows][cols] == "U") and (hit not in computer_ships.values()):
            #Checks if the shot is a mine
            if effects_of_mine(hit, affected_areas, rows, cols, board, computer_ships, user_ships):
                break
