...
def effects_of_mine(hit, affected_areas, rows, cols, board, computer_ships, user_ships):
   """This function plays out certain tasks if a mine was hit. It takes in the parameters
   hit, affected_areas, rows, cols, board, computer_ships, user_ships and return True or False accordingly.
   It also makes changes on the board if any changes need to be made.
   """
...
def make_user_move(board, board_size,computer_ships, affected_areas, user_ships):
 '''
 Handles the user's missile input and lets them know if it was a hit or miss.
 Takes in the parameter board, board_size, and computer_ship.
 '''
 ...
 while not valid_move:
    try:
        ...
        if ((0<=(row)<board_size) and (0<=(col)<board_size)) and board[rows][cols] == " ":
            ...
            print("\nInvalid input. Please enter a position within the range or enter a position that is currently empty.") 
...
def make_computer_move(board, board_size, user_ships, affected_areas, computer_ships):
 '''
 Randomly generates where the computer's missiles will fall and updates the user.
 Takes in the parameters board, board_size, and user_ships.
 '''
 while True:
...
def validate_board_size(board_size):
 ...
 Will validate the board size and make sure it is within 6 and 10'''
...
def gameboard_squares(board_size):
 ...
 Takes in the parameters board_size.
...
def display_board(board, user_ship, computer_ships, mines):
 '''
 Displays the state of the game with the user's and computer's ship positions.
 Takes in the parameters board, user_ship, and computer_ship.
 '''
 print("  ", end=" ")
 ...
 for row in range(length):
    ...
    for col in range(length):
        ...
        if ship in computer_ships.values() and board[row][col] == " ":
            board[row][col] = "C" #Shows the user ships to the user
        if ship in mines.keys() and board[row][col] == " ":
           board[row][col] = "M"
...
def winner(user_ships, computer_ships):
 '''
 Prints the outcome of the game as tie, user won, or computer won 
 based on the number of ships. Takes in user_ships and computer_ships as parameters.
 '''
...
def place_mines( user_ships, computer_ships, board_size):
 '''
 Randomly places 1-3 mines on the board.
 Takes in user_ships, computer_ships, and board_size as parameters.
 '''
...
def mine_damage(mines):
 '''
 Calculates the area that is damaged by each type of mine.
 Takes in mine as a parameter.
 '''
...
def welcome_message():
 """Prints the instructions and welcome message 
 for the user's ease"""
...
def game(turn, user_ships, computer_ships, board, board_size, affected_areas, mines):
    """This function takes in the parameters turn, user_ships, computer_ships, board, board_size, affected_areas.
    It then uses the functions to allow the players to take turns and calls the appropriate functions as necessary.
    """
    ...
    while ((len(user_ships))>0) and ((len(computer_ships))>0):
        if turn == "Y":
            ...
            display_board(board, user_ships, computer_ships, mines)
            turn = "N" 
        elif turn == "N":
...
def get_board_information():
 ...
 needed to make the game's gameboard and returns bard and board_size"""
...
def main():
 ...
 display_board(board,user_ships, computer_ships, mines)  #Displays the board

 #Asks the user if they want to go first
 turn = validate_first()
 game(turn, user_ships, computer_ships, board, board_size, affected_areas, mines) #Begins game
